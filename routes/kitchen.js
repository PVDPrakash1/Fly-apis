...
router.post("/login", async (req, res) => {
  ...
  try {
    ...
    const user = await Kitchen.findOne({ username });
    console.log(user);
...
router.get("/all", async function (req, res, next) {
  try {
    ...
    const waiters = await Kitchen.aggregate()
      .sort({ createdAt: -1 })
      .skip(skip)
      .limit(limit);

    const totalCount = await Kitchen.countDocuments({});
...
router.post(
  ...
  [
    body("name").custom(async (value, { req }) => {
      ...
      const existingCategories = await Kitchen.findOne({ name: value });
  ...
  async function (req, res, next) {
    ...
    try {
      ...
      res.json({
        ...
        data: { _id: kitchen._id, name: kitchen.name },
      });
    } catch (error) {
      res.status(500).json({ message: error.message });
    }
